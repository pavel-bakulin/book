...
function __construct($host,$base_login,$base_passw,$base_name){
 $this->db = @mysql_connect($host,$base_login,$base_passw) or die("<p align='left'>Невозможно соединиться с сервером</p>");
...
public function insupd_sql($sql){
 @mysql_query($sql,$this->db) or die("don't work query ".$sql);
}
